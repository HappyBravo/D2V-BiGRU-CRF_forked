...
class IndexTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None, d2v_vectors=None):
        ...
        max_sent_len = len(word_ids[0])
        dvecs = [[0.0]*max_sent_len]*len(d2v_vectors)
        for sntind,d2vec in enumerate(d2v_vectors):
            for wrdind in range(max_sent_len):
                # print(d2vec)
                dvecs[sntind][wrdind] = d2vec
        d2v_vectors = np.array(dvecs).astype('float32')

        if self._use_char:
            char_ids = [[self._char_vocab.doc2id(w) for w in doc] for doc in X]
            char_ids = pad_nested_sequences(char_ids)
            features = [word_ids, char_ids, d2v_vectors]
        else:
            features = [word_ids, d2v_vectors]
